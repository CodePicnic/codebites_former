define('console', [], function() {
  ...
  function preJSON(object) {
    ...
    if (
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        elementProperties[key] = preJSON.call(this, value);
      }

      linearObject[object.tagName.toLowerCase()] = elementProperties;
    }
    else if (
      ...
      for (var i = 0; i < object.length; i++) {
        ...
        linearObject.push(preJSON.call(this, value));
      }
    }
      ...
      for (var i = 0; i < keys.length; i++) {
        var key = keys[i];
        linearObject[key] = preJSON.call(this, object[key]);
      }
    }
  ...
  function inspectArray(array, rootClass) {
    ...
    for (var i = 0; i < array.length; i++) {
      listContent += '<dd>' + inspect.call(this, array[i]) + '</dd><br/>';
    }
  ...
  function inspectObject(object, rootClass) {
    ...
    for (var i = 0; i < keys.length; i++) {
      var key = keys[i];
      listContent += '<dt>' + key + ':</dt>' +
                      '<dd>' + inspect.call(this, object[key]) + '</dd><br/>';
    }
  ...
  function inspect(content, rootClass) {
    ...
    else if (content instanceof this.Date) {
      inspectedContent = '<span class="date">' + content.toString() + '</span>';
    }
    else if (Array.isArray(content)) {
      inspectedContent = inspectArray.call(this, content, rootClass);
    }
    else if (typeof content === 'object') {
      inspectedContent = inspectObject.call(this, content, rootClass);
    }
    else {
