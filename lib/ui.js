define('ui', ['lang_mappings', 'console', 'runner'], function(LangMappings, Console, BiteRunner) {
  ...
  function addTab(tabs, bite) {
    ...
    tabs.innerHTML += '<a href="#" class="bite-container-tab" data-mode="' + biteMode + '">' + biteName + '<small class="bite-container-remove-tab" title="Cerrar">✖</small></a>';
  }
  ...
  function addBite(group, mode) {
    ...
    var tab = tabs.querySelector('[data-mode="' + mode + '"]');
  ...
  function bindEvents(group) {
    ...
    tabs.addEventListener('dblclick', function(e) {
      ...
      if (biteTabs.length === group.bites.length &&
        e.target.classList.contains('bite-container-tab') &&
        e.target.dataset.mode) {
        
        var newName = prompt('¿Cambiar nombre?'),
            index = Array.prototype.indexOf.call(biteTabs, e.target),
            bite = group.bites[index];
        ...
        e.target.firstChild.textContent = newName;
      }
    });
    ...
    tabs.addEventListener('click', function(e) {
      if (e.target.classList.contains('bite-container-tab')) {
        ...
        switch(e.target.id) {
          ...
          default:
            ...
            if (group.bites.length === biteTabs.length && bite) {
              ...
              for (var i = 0; i < biteTabs.length; i++) {
                biteTabs[i].classList.remove('active');
              }
        ...
        removeBite(group, e.target.parentElement);
      }
    });
